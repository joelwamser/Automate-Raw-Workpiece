...
// CREATE CONSTANT-SIZE BOX GEOMETRY - Function to create brep
// geometry for a 10 by 5 by 3 rectangular box aligned along the XYZ
// axes.  The top face is centered at the origin and is 10 by 5, the
// depth of box is 3.  As with all STEP geometry, the units are given
// by the context of the enclosing shape representation, so this can
// be used to make inch boxes, millimeter boxes, or any other unit.
// The function creates six faces and connects them using topology.
//
...
{
    // faces made in the order top, bottom, front, back, right. left
    ...
    torigin->coordinates ()->add(0)->add(0)->add(0);
    ...
    stp_edge_loop *loop1 = pnew stp_edge_loop;
    loop1->edge_list()->add (tedge1)->add(tedge2)->add(tedge3)->add (tedge4);

    stp_face_outer_bound *tbnd1 = pnew stp_face_outer_bound;
    tbnd1->bound (loop1);
    tbnd1->orientation (1);

    tface1->bounds()->add (tbnd1);
    ...
    borigin->coordinates ()->add(0)->add(0)->add(-3);		// z is - 3
    ...
    bloop1->edge_list()->add (bedge4)->add(bedge3)->add(bedge2)->add (bedge1);
    ...
    // No new points needed and some edges can be reused
    ...
    // New edge from (-5, 2.5, 0) to  (-5, 2.5, -3)
    ...
    forigin->coordinates ()->add(0)->add(-2.5)->add(-1.5);
    ...
    floop1->edge_list()->add (fedge1)->add(fedge2)->add(fedge3)->add (fedge4);
    ...
    // (-5, 2.5, 0) to (5, 2.5, 0) to (5, 2.5, -3) to (-5, 2.5, -3)
    ...
    korigin->coordinates ()->add(0)->add(2.5)->add(-1.5);
    ...
    kloop1->edge_list()->add (kedge4)->add(kedge3)->add(kedge2)->add (kedge1);
    ...
    // Make two new planes and add topology with all edges reversed from original (required by standard)

    // Left face
    // (-5, -2.5, 0) to (-5, 2.5, 0) to (-5, 2.5, -3) to (-5, -2.5, -3)

    // lln1 already exists as ln2/ec2 (reversed)
    ...
    lorigin->coordinates ()->add(-5)->add(0)->add(-1.5);
    ...
    rorigin->coordinates ()->add(5)->add(0)->add(-1.5);
    stp_direction *rz_axis = pnew stp_direction;
    rz_axis->direction_ratios()->add(-11)->add(0)->add(0);
    ...
    rloop1->edge_list()->add (redge4)->add(redge3)->add(redge2)->add (redge1);
    ...
    color_face (tface1, "yellow");		// red green yellow cyan magenta black white 
    color_face (fface1, "magenta");
    color_face (rface1, "green");
